...
document.querySelectorAll('.showroom-scroll').forEach(scroller=>{
  ...
  scroller.addEventListener('pointermove',e=>{
    ...
    scroller.scrollLeft=scrollLeft-(x-startX)*1.15;
  });
});

// animate details open smoothly feel
...
document.querySelectorAll('.showroom-slide').forEach(slide=>{
  slide.addEventListener('click', ()=>{
    ...
    if(!lightbox){
      ...
      lightbox.innerHTML = '<button class="lightbox-close" type="button">×</button><img src="" alt="Réalisation Amine Cars"><p></p>';
...
// Horizontal showroom: drag + active center
document.querySelectorAll('.showroom-scroll').forEach(scroller=>{
  ...
  scroller.addEventListener('scroll', ()=>{
    window.requestAnimationFrame(markCenter);
  }, {passive:true});
